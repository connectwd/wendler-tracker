...
describe("checkPlateau", () => {
  it("does not flag steady improvement", () => {
    const workouts = [
      ...
      week3Workout("c4", "bench", 185),
      week3Workout("c5", "bench", 192),
    ];
    expect(checkPlateau("bench", 5, cycles, workouts, "kg").isPlateaued).toBe(
      false,
    );
  });
  ...
  it("flags 3 flat cycles", () => {
    const workouts = [
      week3Workout("c3", "bench", 185),
      week3Workout("c4", "bench", 184.5),
      week3Workout("c5", "bench", 185.2),
    ];
    expect(checkPlateau("bench", 5, cycles, workouts, "kg").isPlateaued).toBe(
      true,
    );
  ...
  it("flags a decline", () => {
    const workouts = [
      week3Workout("c3", "bench", 190),
      week3Workout("c4", "bench", 185),
      week3Workout("c5", "bench", 178),
    ];
    expect(checkPlateau("bench", 5, cycles, workouts, "kg").isPlateaued).toBe(
      true,
    );
  ...
  it("a dip followed by recovery is not a plateau - one improvement step breaks the streak", () => {
    const workouts = [
      week3Workout("c3", "bench", 190),
      week3Workout("c4", "bench", 182),
      week3Workout("c5", "bench", 195),
    ];
    expect(checkPlateau("bench", 5, cycles, workouts, "kg").isPlateaued).toBe(
      false,
    );
  ...
  it("is scoped per lift - one lift plateauing does not affect another", () => {
    ...
    expect(checkPlateau("squat", 5, cycles, workouts, "kg").isPlateaued).toBe(
      true,
    );
  });
  ...
  it("the reason message respects the unit setting - this was hardcoded to kg before", () => {
    const workouts = [
      week3Workout("c3", "bench", 100),
      week3Workout("c4", "bench", 100),
      week3Workout("c5", "bench", 100),
    ];
    expect(checkPlateau("bench", 5, cycles, workouts, "kg").reason).toMatch(
      /kg\)$/,
    );
    expect(checkPlateau("bench", 5, cycles, workouts, "lb").reason).toMatch(
      /lb\)$/,
    );
  });
});
