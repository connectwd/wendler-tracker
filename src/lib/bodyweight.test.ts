...
function workout(date: string | null, liftId: string, e1rm: number | null): Workout {
  return {
    id: `${liftId}-${date}`,
    cycleId: 'c1',
    liftId,
...
describe('sortedByDate', () => {
  it('sorts oldest first regardless of input order', () => {
    const result = sortedByDate([entry('2026-03-03', 1), entry('2026-03-01', 2), entry('2026-03-02', 3)]);
...
describe('latestBodyweight', () => {
  ...
  it('returns the most recent entry regardless of input order', () => {
    const result = latestBodyweight([entry('2026-03-01', 90), entry('2026-03-15', 89), entry('2026-03-08', 89.5)]);
    expect(result).toEqual(entry('2026-03-15', 89));
  });
});

describe('bodyweightAsOf', () => {
  const history = [entry('2026-01-01', 92), entry('2026-02-01', 90), entry('2026-03-01', 88)];
...
describe('bodyweightTrendPoints', () => {
  it('produces sequential x values in chronological order', () => {
    const points = bodyweightTrendPoints([entry('2026-03-08', 89.5), entry('2026-03-01', 90)]);
...
describe('strengthToBodyweightRatioPoints', () => {
  ...
  it('divides e1RM by the carried-forward bodyweight as of that session date', () => {
    ...
    // 90 / 90 = 1
    expect(strengthToBodyweightRatioPoints(workouts, entries, 'bench')).toEqual([{ x: 0, y: 1 }]);
  });
  ...
  it('orders points chronologically by workout date, not input order', () => {
    const workouts = [workout('2026-03-01', 'bench', 102), workout('2026-02-01', 'bench', 100)];
    const entries = [entry('2026-01-01', 100)];
...
describe('hasAnyRatioData', () => {
  ...
  it('is true once at least one lift has a computable point', () => {
    const workouts = [workout('2026-02-01', 'bench', 100), workout('2026-02-01', 'squat', 150)];
    const entries = [entry('2026-01-01', 100)];
