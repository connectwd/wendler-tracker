...
- Comments in the calculation layer explain _why_, several tied to verified real numbers rather than restating the code.
...
**FIXED.** `backup.ts` now rejects a backup with an _older_ schema version too (previously only newer was rejected), with a message explaining why, instead of silently corrupting the app on the next render.
