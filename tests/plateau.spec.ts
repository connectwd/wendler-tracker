...
test("three flat cycles trigger the plateau warning with a working reset option", async ({
  page,
}) => {
  ...
  for (const cycleNum of [1, 2]) {
    await fastForwardCycle(page, [
      { liftName: "Bench Press", week: 3, amrapReps: 3 },
    ]);
    await page.getByTestId("start-next-cycle-btn").click();
    await expect(page.getByTestId("plateau-warning-Bench Press")).toHaveCount(
      0,
    );
    await page.getByTestId("confirm-next-cycle-btn").click();
    await expect(
      page.getByText(`Cycle ${cycleNum + 1}`, { exact: true }),
    ).toBeVisible();
  }
  ...
  await expect(page.getByTestId("plateau-warning-Overhead Press")).toHaveCount(
    0,
  );
