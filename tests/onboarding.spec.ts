...
test.describe("onboarding", () => {
  test("completing the wizard with default lifts lands on Cycle 1 dashboard", async ({
    page,
  }) => {
    await page.goto("./");
  ...
  test("a direct 1RM entry (1 rep) computes TM as exactly 90% with no Brzycki adjustment", async ({
    page,
  }) => {
    await page.goto("./");
    await page.getByRole("button", { name: "Continue" }).click(); // step 0 -> 1
    ...
    await expect(
      benchCard.getByText("Suggested Training Max (90%): 166.5kg"),
    ).toBeVisible();
  ...
  test("overriding the suggested Training Max applies the typed value instead", async ({
    page,
  }) => {
    await page.goto("./");
    await page.getByRole("button", { name: "Continue" }).click();
  ...
  test("a fifth custom lift can be added and carries through to the dashboard", async ({
    page,
  }) => {
    await page.goto("./");
    await page.getByRole("button", { name: "Continue" }).click();
  ...
  test("a lift can be removed as long as at least one remains", async ({
    page,
  }) => {
    await page.goto("./");
    await page.getByRole("button", { name: "Continue" }).click();
