...
function setUpFakes(opts: { hasController: boolean; supportsServiceWorker?: boolean }) {
  ...
  const fakeNavigator = opts.supportsServiceWorker === false ? {} : { serviceWorker: swContainer };
  vi.stubGlobal('navigator', fakeNavigator);

  const fakeWindow = Object.assign(new FakeEventTarget(), { location: { reload: vi.fn() } });
  vi.stubGlobal('window', fakeWindow);

  const fakeDocument = Object.assign(new FakeEventTarget(), { visibilityState: 'visible' as string });
...
describe('registerServiceWorker', () => {
  ...
  it('does nothing when the browser has no serviceWorker support', () => {
    const { fakeWindow } = setUpFakes({ hasController: false, supportsServiceWorker: false });
  ...
  it('registers on window load in production', async () => {
    ...
    await vi.waitFor(() => expect(swContainer.register).toHaveBeenCalledTimes(1));
    expect(swContainer.register.mock.calls[0][0]).toMatch(/sw\.js$/);
    expect(swContainer.register.mock.calls[0][1]).toEqual({ updateViaCache: 'none' });
  });
  ...
  it('calls onUpdateAvailable when the page already had a controller and a new one takes over (genuine update)', async () => {
    ...
    await vi.waitFor(() => expect(swContainer.hasListener('controllerchange')).toBe(true));
  ...
  it('polls registration.update() on an interval', async () => {
    vi.useFakeTimers();
    const { fakeWindow, fakeRegistration } = setUpFakes({ hasController: false });
    registerServiceWorker(vi.fn(), false);
    fakeWindow.dispatch('load');
  ...
  it('re-checks for an update when the tab becomes visible again', async () => {
    const { fakeWindow, fakeDocument, fakeRegistration } = setUpFakes({ hasController: false });
    registerServiceWorker(vi.fn(), false);
    fakeWindow.dispatch('load');
    await vi.waitFor(() => expect(fakeDocument.hasListener('visibilitychange')).toBe(true));
