...
test.describe('editing a lift\'s Training Max after onboarding', () => {
  test('correcting a TM in Settings recalculates pending workouts in the current cycle', async ({ page }) => {
    ...
    await expect(page.getByTestId('main-target-weight-0')).toContainText('97.5');
    await expect(page.getByTestId('main-target-weight-1')).toContainText('112.5');
    await expect(page.getByTestId('main-target-weight-2')).toContainText('127.5');
  });

  test('dismissing the confirmation leaves the Training Max unchanged', async ({ page }) => {
  ...
  test('the "work it out from a recent lift" calculator suggests a TM the same way onboarding does', async ({
    ...
    await expect(page.getByTestId('main-target-weight-0')).toContainText('87.5');
    await expect(page.getByTestId('main-target-weight-1')).toContainText('100');
    await expect(page.getByTestId('main-target-weight-2')).toContainText('112.5');
  });
