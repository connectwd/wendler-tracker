...
test.describe("GitHub sync", () => {
  test("connecting with valid credentials pushes the initial data and shows Synced", async ({
    page,
  }) => {
  ...
  test("saving a workout after sync is enabled triggers an automatic push, no manual step", async ({
    page,
  }) => {
    const remote = createFakeGitHubRemote();
    await attachGitHubMock(page, remote);
  ...
  test('a genuine two-device conflict surfaces the resolution screen, and "keep this device" wins cleanly', async ({
    browser,
  }) => {
    const remote = createFakeGitHubRemote();
    ...
    await configureGitHubSync(pageB, {
      owner: "jake",
      repo: "wendler-data",
      token: "fake-token",
    });
    ...
    await expect(pageB.getByText("Two versions to choose from")).toBeVisible({
      timeout: 6000,
    });
    await expect(pageB.getByText("This device")).toBeVisible();

    await pageB
      .getByRole("button", { name: "Keep this device's data" })
      .click();
    await expect(pageB.getByText("Two versions to choose from")).toHaveCount(0);
