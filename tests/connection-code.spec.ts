...
test.describe('GitHub sync connection code', () => {
  test('before connecting, only the paste-code option is offered (not the generate/export option)', async ({ page }) => {
  ...
  test('after connecting, only the generate/export option is offered (not the paste-code option)', async ({ page }) => {
    const remote = createFakeGitHubRemote();
    await attachGitHubMock(page, remote);
    await completeOnboarding(page);
  ...
  test('generating a code on device A and pasting it on device B fills in the same fields and connects', async ({ browser }) => {
    const remote = createFakeGitHubRemote();
    ...
    await configureGitHubSync(pageA, { owner: 'jake', repo: 'wendler-data', token: 'fake-token-xyz', path: 'wendler-data.json' });
    ...
    await expect(pageB.getByTestId('sync-repo-input')).toHaveValue('wendler-data');
    await expect(pageB.getByTestId('sync-path-input')).toHaveValue('wendler-data.json');
    await expect(pageB.getByTestId('sync-token-input')).toHaveValue('fake-token-xyz');
  ...
  test('pasting a garbage code shows an inline error and leaves the manual fields untouched', async ({ page }) => {
    const remote = createFakeGitHubRemote();
    await attachGitHubMock(page, remote);
    await completeOnboarding(page);

    await page.getByRole('button', { name: 'Settings' }).click();
    await page.getByTestId('sync-code-input').fill('this-is-not-a-valid-connection-code!!!');
  ...
  test('pasting well-formed base64/JSON that is missing a required field also shows an inline error', async ({ page }) => {
    const remote = createFakeGitHubRemote();
    await attachGitHubMock(page, remote);
    await completeOnboarding(page);

    await page.getByRole('button', { name: 'Settings' }).click();
    const incompleteCode = Buffer.from(JSON.stringify({ owner: 'jake', repo: 'wendler-data' }), 'utf-8').toString('base64');
  ...
  test('applying a valid code clears the paste field so a stale code isn\'t left sitting there', async ({ page }) => {
    const remote = createFakeGitHubRemote();
    await attachGitHubMock(page, remote);
    await completeOnboarding(page);
    ...
    const code = Buffer.from(
      JSON.stringify({ owner: 'jake', repo: 'wendler-data', path: 'wendler-data.json', token: 'fake-token' }),
      'utf-8'
    ).toString('base64');
  ...
  test('copy to clipboard button copies the exact connection code shown in the textarea', async ({ page, context }) => {
    ...
    await page.getByRole('button', { name: 'Settings' }).click();
    await configureGitHubSync(page, { owner: 'jake', repo: 'wendler-data', token: 'fake-token-copy' });
    await expect(page.getByTestId('sync-status-pill')).toHaveText(/Synced/, { timeout: 6000 });
  ...
  test('the connection-code warning about it not being encrypted is shown alongside the generated code', async ({ page }) => {
    const remote = createFakeGitHubRemote();
    await attachGitHubMock(page, remote);
    await completeOnboarding(page);
  ...
  test('disabling sync hides a previously shown connection code', async ({ page }) => {
    const remote = createFakeGitHubRemote();
    await attachGitHubMock(page, remote);
    await completeOnboarding(page);
