...
describe("availablePlates", () => {
  it("kg and lb are genuinely different plate sets", () => {
    expect(availablePlates("kg").map((p) => p.weight)).toEqual([
      25, 20, 15, 10, 5, 2.5, 1.25,
    ]);
    expect(availablePlates("lb").map((p) => p.weight)).toEqual([
      45, 35, 25, 10, 5, 2.5,
    ]);
  });
});
...
describe("calculatePlates", () => {
  it("kg: 85kg over a 20kg bar", () => {
    ...
    expect(calculatePlates(85, 20, "kg").perSide.map((p) => p.weight)).toEqual([
      25, 5, 2.5,
    ]);
  });
  ...
  it("kg: 107.5kg over a 20kg bar", () => {
    ...
    expect(
      calculatePlates(107.5, 20, "kg").perSide.map((p) => p.weight),
    ).toEqual([25, 15, 2.5, 1.25]);
  });

  it("lb: uses the lb plate set, not kg plates mislabeled", () => {
    // (235-45)/2 = 95/side -> 45 + 45 + 5
    expect(calculatePlates(235, 45, "lb").perSide.map((p) => p.weight)).toEqual(
      [45, 45, 5],
    );
  });
