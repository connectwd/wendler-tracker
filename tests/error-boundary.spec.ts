...
test.describe("error boundary", () => {
  test("a render crash shows the fallback screen, not a blank page", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("The app crashed, not your data"),
    ).toBeVisible();
  });

  test("data logged before the crash is untouched underneath it", async ({
    page,
  }) => {
    ...
    await page.goto("./");
    await expect(
      page.getByText("Cycle 1", { exact: true }).first(),
    ).toBeVisible();
  });

  test("exporting a backup from the crash screen still works", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.goto("./?__crashtest=boundary");
    ...
    expect(download.suggestedFilename()).toMatch(
      /^wendler-tracker-backup-\d{4}-\d{2}-\d{2}\.json$/,
    );
    ...
    await expect(page.getByTestId("error-boundary-export-btn")).toHaveText(
      "Exported ✓",
    );
  });
  ...
  test("technical details are available but collapsed by default", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.goto("./?__crashtest=boundary");
