...
test.describe("rest timer", () => {
  ...
  test("warm-up and BBS/accessory sections default to the short duration, main defaults to the long one", async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId("rest-trigger-accessory")).toContainText(
      "90s",
    );
  });

  test("starting a rest from the warm-up section opens the timer at the short default", async ({
    page,
  }) => {
  ...
  test("starting a rest from main work opens the timer at the long default", async ({
    page,
  }) => {
  ...
  test("skip rest closes the overlay and returns to the session with nothing lost", async ({
    page,
  }) => {
  ...
  test("the mini-game canvas and flap control are present and clickable while resting", async ({
    page,
  }) => {
  ...
  test('reaching zero shows "Rest\'s over!" and the button changes to "Back to workout"', async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId("rest-timer-close-btn")).toHaveText(
      "Back to workout",
    );
  });

  test("a completed session still saves correctly after opening and closing a rest timer mid-session", async ({
    page,
  }) => {
    await page.getByTestId("rest-trigger-warmup").click();
    await page.getByTestId("rest-timer-close-btn").click();
