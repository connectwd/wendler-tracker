...
describe("main work percentages", () => {
  ...
  it("week 1 (65/75/85%)", () => {
    expect(
      generateMainWorkSets(tm, 1, round).map((s) => s.targetWeight),
    ).toEqual([107.5, 125, 142.5]);
  });
  it("week 2 (70/80/90%)", () => {
    expect(
      generateMainWorkSets(tm, 2, round).map((s) => s.targetWeight),
    ).toEqual([117.5, 132.5, 150]);
  });
  it("week 3 (75/85/95%)", () => {
    expect(
      generateMainWorkSets(tm, 3, round).map((s) => s.targetWeight),
    ).toEqual([125, 142.5, 157.5]);
  });
  it("week 4 deload (40/50/60%)", () => {
    expect(
      generateMainWorkSets(tm, 4, round).map((s) => s.targetWeight),
    ).toEqual([67.5, 82.5, 100]);
  });
  ...
  it("deload week has no AMRAP set", () => {
    expect(generateMainWorkSets(tm, 4, round).every((s) => !s.isAmrap)).toBe(
      true,
    );
  });
});
...
describe("cycle and workout generation", () => {
  ...
  it("builds cycle 1 with the given Training Maxes", () => {
    const cycle = buildFirstCycle(
      lifts,
      { bench: 166.5, squat: 108 },
      "2026-07-14",
    );
...
describe("regenerateWorkoutForNewTrainingMax (correcting a Training Max mid-cycle)", () => {
  ...
  it("recomputes every set to match the corrected TM when nothing has been logged yet", () => {
    ...
    expect(corrected.mainSets.map((s) => s.targetWeight)).toEqual([
      97.5, 112.5, 127.5,
    ]);
    expect(
      corrected.warmupSets.every(
        (s) => s.targetWeight < corrected.mainSets[0].targetWeight,
      ),
    ).toBe(true);
  ...
  it("leaves already-completed sets exactly as logged, only touching pending ones", () => {
    const workout = freshWorkout(1);
    const loggedFirstMainSet = {
      ...workout.mainSets[0],
      completed: true,
      actualWeight: 107.5,
      actualReps: 5,
    };
    const partiallyLogged: Workout = {
      ...workout,
      mainSets: [loggedFirstMainSet, workout.mainSets[1], workout.mainSets[2]],
    };

    const corrected = regenerateWorkoutForNewTrainingMax(
      partiallyLogged,
      150,
      round,
    );
  ...
  it("preserves week-specific rep scheme and AMRAP flag while updating weight", () => {
    ...
    expect(corrected.mainSets.map((s) => s.isAmrap)).toEqual([
      false,
      false,
      true,
    ]);
