...
const validCycle = { id: 'c1', cycleNumber: 1, trainingMaxes: { l1: 100 }, status: 'active' };
const validWorkout = {
  id: 'w1',
  liftId: 'l1',
  cycleId: 'c1',
...
const validBodyweightEntry = { id: '2026-01-01', date: '2026-01-01', weight: 90 };
const validAppData = {
...
describe('getLastBackupTimestamp / daysSinceLastBackup', () => {
  ...
  it('reports 0 for a timestamp from a few minutes ago', () => {
    localStorage.setItem('wendler-tracker-last-backup-at', new Date(Date.now() - 5 * 60 * 1000).toISOString());
    expect(daysSinceLastBackup()).toBe(0);
  });

  it('reports whole days elapsed, rounding down rather than up', () => {
    const fourteenDaysAgo = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000 - 60 * 1000);
    localStorage.setItem('wendler-tracker-last-backup-at', fourteenDaysAgo.toISOString());
    expect(daysSinceLastBackup()).toBe(14);
  });

  it('does not round 13 days 23 hours up to 14 - the nag banner threshold matters here', () => {
    const almostFourteen = new Date(Date.now() - (14 * 24 * 60 * 60 * 1000 - 60 * 60 * 1000));
    localStorage.setItem('wendler-tracker-last-backup-at', almostFourteen.toISOString());
...
describe('per-record validators', () => {
  ...
  it('isValidWorkout requires every set array to actually be an array, not just present', () => {
    ...
    expect(isValidWorkout({ ...validWorkout, status: 'in-progress' })).toBe(false);
  });

  it('isValidBodyweightEntry requires a numeric weight', () => {
    expect(isValidBodyweightEntry(validBodyweightEntry)).toBe(true);
    expect(isValidBodyweightEntry({ ...validBodyweightEntry, weight: '90' })).toBe(false);
  });
  ...
  it('isValidAppData rejects a data object where every top-level container looks right but one record inside is malformed', () => {
    expect(isValidAppData({ ...validAppData, workouts: [{ ...validWorkout, mainSets: undefined }] })).toBe(false);
  });
...
describe('importBackupFromFile', () => {
  it('rejects a file that is not valid JSON', async () => {
    await expect(importBackupFromFile(fileOf('{not json'))).rejects.toThrow(BackupValidationError);
    await expect(importBackupFromFile(fileOf('{not json'))).rejects.toThrow(/not valid JSON/i);
  });

  it("rejects a JSON file that isn't a wendler-tracker backup", async () => {
    const file = fileOf(JSON.stringify({ app: 'some-other-app', data: validAppData, schemaVersion: SCHEMA_VERSION }));
    await expect(importBackupFromFile(file)).rejects.toThrow(/doesn't look like a wendler-tracker backup/i);
  });
  ...
  it('rejects a backup from a newer schema version than this build supports', async () => {
    ...
    await expect(importBackupFromFile(file)).rejects.toThrow(/newer version of the app/i);
  });

  it('rejects a backup from an older schema version with a message telling them to keep the file', async () => {
    const file = fileOf(validBackupFile({ schemaVersion: SCHEMA_VERSION - 1 }));
    await expect(importBackupFromFile(file)).rejects.toThrow(/older version of the app/i);
    await expect(importBackupFromFile(file)).rejects.toThrow(/hold onto the file/i);
  });

  it('rejects a backup whose data does not match the expected shape', async () => {
    const file = fileOf(validBackupFile({ data: { ...validAppData, lifts: 'not an array' } }));
  ...
  it('fills in an empty bodyweightEntries array for a pre-bodyweight-feature backup rather than rejecting it', async () => {
    ...
    expect(persist).toHaveBeenCalledWith({ ...dataWithoutBodyweight, bodyweightEntries: [] });
  });
});
