...
test.describe("workout logging", () => {
  ...
  test("a fresh Bench Press week 1 session shows the correct warm-up, main, and BBS prescriptions", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator(".set-row").filter({ hasText: "40kg × 5" }),
    ).toBeVisible();
    await expect(
      page.locator(".set-row").filter({ hasText: "50kg × 5" }),
    ).toBeVisible();
    await expect(
      page.locator(".set-row").filter({ hasText: "60kg × 3" }),
    ).toBeVisible();
    await expect(
      page.locator(".set-row").filter({ hasText: "65kg × 5" }),
    ).toBeVisible();
    await expect(
      page.locator(".set-row").filter({ hasText: "75kg × 5" }),
    ).toBeVisible();
    await expect(
      page.locator(".set-row").filter({ hasText: "85kg × 5+" }),
    ).toBeVisible();
    await expect(
      page.getByText("Boring But Strong — 10 × 5 @ 65kg"),
    ).toBeVisible();
  });

  test('there is no "last cycle" comparison in cycle 1 (no history to compare against)', async ({
    page,
  }) => {
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test("entering AMRAP reps shows the live estimated 1RM and completes that set", async ({
    page,
  }) => {
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test("saving a fully logged session marks it Done on the dashboard", async ({
    page,
  }) => {
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test("the plate breakdown for the 85kg top set shows the right plates per side over a 20kg bar", async ({
    page,
  }) => {
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test("the BBS counter cannot go below 0 or above the total set count", async ({
    page,
  }) => {
    await page.getByTestId("workout-card-Bench Press").click();
