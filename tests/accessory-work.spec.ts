...
test.describe("accessory work", () => {
  ...
  test("a fresh session shows the picker, capped at 3 selections", async ({
    page,
  }) => {
    await page.getByTestId("workout-card-Bench Press").click();

    await expect(page.getByTestId("accessory-selected-count")).toHaveText(
      "0/3 selected",
    );
    ...
    await page.getByTestId("accessory-option-dips").click();
    await expect(
      page.getByTestId("accessory-option-pushups"),
    ).not.toBeDisabled();
  });

  test("confirming the selection replaces the picker with input sections for those exercises", async ({
    page,
  }) => {
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test("logging sets, saving, and reopening the session preserves the values", async ({
    page,
  }) => {
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test('"Change exercises" returns to the picker with the current picks still checked', async ({
    page,
  }) => {
    await page.getByTestId("workout-card-Bench Press").click();
    await selectAccessories(page, ["dips"]);
    ...
    await expect(page.getByTestId("accessory-option-dips")).toHaveClass(
      /checked/,
    );
    await expect(page.getByTestId("accessory-selected-count")).toHaveText(
      "1/3 selected",
  ...
  test("a later session for the same lift remembers the previous pick instead of showing the picker again", async ({
    page,
  }) => {
    await page.getByTestId("workout-card-Bench Press").click();
    await selectAccessories(page, ["dips"]);
  ...
  test("history shows the last time this exercise was logged, even several cycles back", async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId("accessory-history-dips")).toContainText(
      "15kg×10",
    );
  });
});
