...
test.describe("bodyweight tracking", () => {
  ...
  test('shows "not logged yet" and no history toggle before anything is logged', async ({
    page,
  }) => {
  ...
  test("logging today's weight shows it as current and adds it to history", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("Show history (1)").or(page.getByText("Hide history")),
    ).toBeVisible();
  });

  test("logging the same date twice overwrites rather than duplicating", async ({
    page,
  }) => {
  ...
  test("logging an older date does not override the current value shown from a more recent entry", async ({
    page,
  }) => {
    ...
    await expect(page.locator("p", { hasText: "Current:" })).toContainText(
      "92",
    );
  });

  test('deleting the only entry falls back to "not logged yet"', async ({
    page,
  }) => {
  ...
  test("Settings shows the same current value read-only and links back to Progress", async ({
    page,
  }) => {
  ...
  test("the strength/bodyweight ratio chart only appears once there is both an e1RM and a bodyweight to divide by", async ({
    page,
  }) => {
