...
describe("describeStorageError", () => {
  it("gives an actionable message for QuotaExceededError", () => {
    expect(describeStorageError(domException("QuotaExceededError"))).toMatch(
      /storage is full/i,
    );
    expect(describeStorageError(domException("QuotaExceededError"))).toMatch(
      /export a backup/i,
  ...
  it("gives an actionable message for InvalidStateError", () => {
    expect(describeStorageError(domException("InvalidStateError"))).toMatch(
      /reloading the page/i,
    );
  });

  it("gives an actionable message for VersionError", () => {
    expect(describeStorageError(domException("VersionError"))).toMatch(
      /older version/i,
    );
  });
  ...
  it("falls back to the DOMException message for an unrecognized name", () => {
    expect(
      describeStorageError(
        domException("UnknownWeirdError", "something specific broke"),
      ),
    ).toBe("something specific broke");
  });

  it("falls back to a generic message for an unrecognized DOMException with no message", () => {
    expect(describeStorageError(domException("UnknownWeirdError"))).toBe(
      "Storage error (UnknownWeirdError).",
    );
  });

  it("uses the message directly for a StorageError", () => {
    expect(
      describeStorageError(new StorageError("could not save your workout")),
    ).toBe("could not save your workout");
  });
  ...
  it("has a final fallback for a thrown non-Error value", () => {
    expect(describeStorageError("a raw string")).toBe(
      "An unknown storage error occurred.",
    );
    expect(describeStorageError(undefined)).toBe(
      "An unknown storage error occurred.",
    );
    expect(describeStorageError({ weird: true })).toBe(
      "An unknown storage error occurred.",
    );
  });
});
