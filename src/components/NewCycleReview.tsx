...
export function NewCycleReview({
  ...
  function amrapSummary(liftId: string): string {
    const liftWorkouts = cycleWorkouts.filter(
      (w) => w.liftId === liftId && w.week !== 4,
    );
  ...
  return (
    <div className="screen">
      <button
        ...
        onClick={goBack}
        style={{ paddingLeft: 0 }}
        data-testid="new-cycle-back-btn"
      >
        ← Back
      </button>
      <p className="eyebrow">
        Cycle {activeCycle.cycleNumber} → {activeCycle.cycleNumber + 1}
      </p>
      <h1>Next Training Maxes</h1>
      <p>
        AMRAP reps shown are weeks 1/2/3 for this cycle — a quick sanity check
        before you roll forward. The suggested TM is just last cycle's TM plus
        your configured increment; edit any of them if a cycle didn't go to
        plan.
      </p>
            ...
            <div className="card" key={lift.id}>
              ...
              <div className="row" style={{ marginTop: 8 }}>
                ...
                <input
                  ...
                  style={{
                    width: 100,
                    borderColor: valid ? undefined : "var(--plate-red)",
                  }}
              ...
              {!valid && (
                ...
                >
                  Enter a positive number, or leave blank to use the suggested{" "}
              ...
              {plateau.isPlateaued && (
                <div
                  ...
                  data-testid={`plateau-warning-${lift.name}`}
                >
                  ...
                  >
                    Reset to{" "}
                    {resetTrainingMax(
                      activeCycle.trainingMaxes[lift.id] ?? 0,
                      0.9,
                    ).toFixed(1)}
