...
test.describe("settings and backup", () => {
  test("changing units/rounding/bodyweight in Settings persists across a reload", async ({
    page,
  }) => {
  ...
  test("importing a backup file fully replaces the current data", async ({
    page,
  }) => {
  ...
  test("importing a file that is not a wendler-tracker backup is rejected with a clear message", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.getByRole("button", { name: "Settings" }).click();
    ...
    await expect(
      page.getByText("doesn't look like a wendler-tracker backup file"),
    ).toBeVisible();
