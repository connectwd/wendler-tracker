...
test.describe("appearance / arcade mode", () => {
  test("toggling Arcade Mode re-themes the app immediately, no save step required", async ({
    page,
  }) => {
  ...
  test("the choice persists across a reload and applies before any settings are touched", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.getByRole("button", { name: "Settings" }).click();
  ...
  test("switching back to Serious Mode restores the original tokens", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.getByRole("button", { name: "Settings" }).click();
  ...
  test("the plate-loading diagram keeps real plate colors regardless of theme", async ({
    page,
  }) => {
    ...
    for (let i = 0; i < count; i++) {
      const bg = await plates
        .nth(i)
        .evaluate((el) => getComputedStyle(el).backgroundColor);
      expect(
        arcadeAccentColors.has(bg),
        `plate color ${bg} matches an arcade accent - token boundary leaked`,
      ).toBe(false);
      expect(
        fixedPlateColors.has(bg),
        `plate color ${bg} isn't one of the fixed literal plate colors`,
      ).toBe(true);
    }
  });
});
