...
test.describe("back navigation", () => {
  test("swiping back from an open workout session returns to the dashboard, not out of the app", async ({
    page,
  }) => {
  ...
  test("the in-app Back button and a swipe both close a clean (non-dirty) session with no prompt", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test("swiping back out of a dirty session prompts to discard, and declining keeps the session open with your entries intact", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test("swiping back out of a dirty session and accepting discards it, same as the Back button", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.getByTestId("workout-card-Bench Press").click();
    ...
    await expect(page.getByTestId("workout-card-Bench Press")).toBeVisible();
    await expect(page.getByTestId("workout-card-Bench Press")).toHaveAttribute(
      "data-status",
      "pending",
    );
  });

  test("saving a session and then swiping back from the dashboard falls through to leaving the page, not a dead swipe", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test("rest timer nests on top of the workout session - one swipe closes just the timer, a second closes the session", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test("swiping back out of the new-cycle review cancels it, same as the Back button", async ({
    page,
  }) => {
    await completeOnboarding(page);
  ...
  test("confirming the new-cycle review rolls forward and a later swipe does not resurface it", async ({
    page,
  }) => {
    await completeOnboarding(page);
    for (const week of [1, 2, 3, 4] as const) {
