...
describe("findLastAccessoryLog", () => {
  it("finds a log from many cycles ago, not just the current one", () => {
    const nineCyclesAgo = makeWorkout("w1", "bench", "2025-10-01", [
      dipsLog(10, 12),
    ]);
    const currentSession = makeWorkout("w3", "bench", "2026-07-15", [
      dipsLog(15, 10),
    ]);

    const last = findLastAccessoryLog(
      "dips",
      [nineCyclesAgo, currentSession],
      "w3",
    );
    expect(last?.date).toBe("2025-10-01");
    expect(last?.exercise.sets[0]).toEqual({
      setNumber: 1,
      weight: 10,
      reps: 12,
    });
...
describe("findLastAccessorySelection", () => {
  ...
  it("is scoped to the right lift - a different lift's selection does not leak in", () => {
    const squatWorkout = makeWorkout("w1", "squat", "2026-07-10", [
      dipsLog(20, 10),
    ]);
