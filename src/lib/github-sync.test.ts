    ...
    vi.stubGlobal(
      ...
      vi.fn(async (_url: string, init?: RequestInit) => {
        ...
        return fakeResponse(200, {
          content: b64(JSON.stringify(remotePayload)),
          sha: "sha-conflict",
        });
      }),
    );
    await expect(pushRemote(config, sampleData, "stale-sha")).rejects.toThrow(
      SyncConflictError,
    );
  });
...
describe("retry with backoff - transient failures only", () => {
  ...
  it("does NOT retry a 409 conflict - it needs the conflict-resolution flow, not a blind retry", async () => {
    ...
    await expect(pushRemote(config, sampleData, "stale")).rejects.toThrow(
      SyncConflictError,
    );
