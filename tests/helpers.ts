...
export const DEFAULT_LIFT_NAMES = [
  "Bench Press",
  "Squat",
  "Deadlift",
  "Overhead Press",
...
export async function completeOnboarding(
  ...
  const lifts: OnboardingLift[] =
    options.lifts ??
    DEFAULT_LIFT_NAMES.map((name) => ({ name, weight: 100, reps: 5 }));
  ...
  if (options.barWeight !== undefined) {
    await page
      .getByLabel(`Bar weight (${units})`)
      .fill(String(options.barWeight));
  }
  if (options.roundingIncrement !== undefined) {
    await page
      .getByLabel(`Round working weights to the nearest (${units})`)
      .fill(String(options.roundingIncrement));
  }
  ...
  for (let i = 0; i < lifts.length; i++) {
    ...
    await card
      .getByLabel(`+ per cycle (${units})`)
      .fill(String(lift.increment));
  }
  await page.getByRole("button", { name: "Continue" }).click();
...
export async function logFullSession(
  ...
  const warmupCount = await page
    .locator('[data-testid^="warmup-check-"]')
    .count();
  for (let i = 0; i < warmupCount; i++) {
    await page.getByTestId(`warmup-check-${i}`).click();
  }

  const mainCheckCount = await page
    .locator('[data-testid^="main-check-"]')
    .count();
  ...
  if (amrapInputVisible > 0 && options.amrapReps === undefined) {
    throw new Error(
      "This session has an AMRAP set - pass amrapReps to logFullSession, or use skipSession().",
    );
  }
...
export async function logAccessorySet(
  ...
  await page
    .getByTestId(`accessory-weight-${exerciseId}-${setIndex}`)
    .fill(String(weight));
  await page
    .getByTestId(`accessory-reps-${exerciseId}-${setIndex}`)
    .fill(String(reps));
}
...
export async function fastForwardCycle(
  ...
  for (const week of ALL_WEEKS) {
    ...
    for (const liftName of liftNames) {
      ...
      await card.click();
      const match = targeted.find(
        (t) => t.liftName === liftName && t.week === week,
      );
      if (match) {
...
export function createFakeGitHubRemote(
...
export async function attachGitHubMock(
  page: Page,
