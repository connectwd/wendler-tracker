...
function ErrorFallback({ error, onReload }: { error: Error; onReload: () => void }) {
  ...
  return (
    <div className="screen" data-testid="error-boundary-screen">
      ...
      <p>
        Everything you've logged is still sitting in this browser's storage - the screen crashed, the database
        didn't. Export a backup now while you're here as a precaution, then reload.
      </p>
      ...
      <div className="stack" style={{ marginTop: 4 }}>
        ...
        >
          {exportState === 'exporting' ? 'Exporting…' : exportState === 'done' ? 'Exported ✓' : 'Export backup now'}
        </button>

        {exportState === 'failed' && (
          <div className="warning-banner" data-testid="error-boundary-export-failed">
            Couldn't export automatically. Your data's still in IndexedDB regardless - check devtools ▸ Application
            ▸ IndexedDB if you need it right away.
          </div>
        )}

        <button className="btn btn-block" onClick={onReload} data-testid="error-boundary-reload-btn">
          Reload app
        </button>
      </div>

      <details style={{ marginTop: 20 }} data-testid="error-boundary-details">
        <summary style={{ color: 'var(--text-faint)', fontSize: 12, cursor: 'pointer' }}>Technical details</summary>
        <pre
          style={{
            whiteSpace: 'pre-wrap',
