...
export function AccessoryWork({
  ...
  const [mode, setMode] = useState<"picker" | "logging">(
    initialAccessories.length > 0 ? "logging" : "picker",
  );
  const [selectedIds, setSelectedIds] = useState<string[]>(
    initialAccessories.map((a) => a.exerciseId),
  );
  const [exercises, setExercises] =
    useState<LoggedAccessoryExercise[]>(initialAccessories);
  ...
  function updateSet(
    exerciseId: string,
    setIndex: number,
    patch: Partial<{ weight: number | null; reps: number | null }>,
  ) {
    setExercises((prev) => {
      ...
      const next = prev.map((ex) =>
        ex.exerciseId === exerciseId
          ? {
              ...
              sets: [
                ...ex.sets,
                { setNumber: ex.sets.length + 1, weight: null, reps: null },
              ],
            }
          : ex,
      );
    ...
    return (
      <div className="card">
        <p style={{ fontSize: 13, marginBottom: 4 }}>
          Wendler's assistance framework: one exercise each from Push, Pull, and
          Single Leg/Core, aiming for {ACCESSORY_REP_TARGET} per category. Pick
          up to {MAX_ACCESSORY_EXERCISES}.
        </p>
        <p
          style={{ fontSize: 12, color: "var(--text-faint)", marginBottom: 8 }}
          data-testid="accessory-selected-count"
        >
          {selectedIds.length}/{MAX_ACCESSORY_EXERCISES} selected
        </p>
        {CATEGORIES.map((cat) => (
          <div key={cat}>
            <div className="accessory-category-label">
              {ACCESSORY_CATEGORY_LABELS[cat]}
            </div>
            {ACCESSORY_CATALOG.filter((o) => o.category === cat).map(
              (option) => {
                const checked = selectedIds.includes(option.id);
                const disabled =
                  !checked && selectedIds.length >= MAX_ACCESSORY_EXERCISES;
                return (
        ...
        >
          {selectedIds.length === 0
            ? "Skip accessories today"
            : `Confirm ${selectedIds.length} exercise${selectedIds.length === 1 ? "" : "s"}`}
        </button>
      </div>
    );
  }
      ...
      <div className="card">
        <div className="row">
          <span style={{ fontSize: 13, color: "var(--text-muted)" }}>
            No accessory work logged today.
          </span>
          <button
            className="btn"
            onClick={() => setMode("picker")}
            data-testid="accessory-change-btn"
          >
            Add some
          </button>
        </div>
  ...
  return (
    <div>
      {exercises.map((ex) => {
        const last = findLastAccessoryLog(
          ex.exerciseId,
          allWorkouts,
          workoutId,
        );
          ...
          <div className="card" key={ex.exerciseId}>
            <div className="row">
              ...
              <span style={{ fontSize: 11, color: "var(--text-faint)" }}>
                {ACCESSORY_CATEGORY_LABELS[ex.category]}
              </span>
            </div>
            {last && (
              <p
                ...
                data-testid={`accessory-history-${ex.exerciseId}`}
              >
                Last time ({last.date}):{" "}
                {last.exercise.sets
