...
test.describe("training overview", () => {
  test("lifetime stats reflect a logged session with the correct tonnage", async ({
    page,
  }) => {
    ...
    const sessionsCard = page
      .locator(".card")
      .filter({ hasText: "sessions logged" });
    await expect(sessionsCard.locator(".big-num")).toHaveText("1");
    const tonnageCard = page
      .locator(".card")
      .filter({ hasText: "lifetime moved" });
    await expect(tonnageCard.locator(".big-num")).toHaveText("5,090kg");
  });

  test("a first-ever AMRAP set on a lift is flagged as a New PR", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.getByTestId("workout-card-Bench Press").click();
  ...
  test("a second AMRAP set that does not beat the first is not flagged as a PR", async ({
    page,
  }) => {
    await completeOnboarding(page);
  ...
  test("the consistency heatmap shows a shaded cell for a logged session", async ({
    page,
  }) => {
    await completeOnboarding(page);
    await page.getByTestId("workout-card-Bench Press").click();
    ...
    await expect(
      page.locator(`.heatmap-cell[title^="${today}: "]`),
    ).toHaveAttribute("data-bucket", "4");
  });
});
