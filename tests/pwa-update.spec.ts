...
test.describe("Update available toast", () => {
  test("does not appear on a fresh install (no prior controller to update from)", async ({
    page,
  }) => {
    await page.goto("./");
  ...
  test("appears once a new service worker takes control of an already-controlled page, and Refresh reloads", async ({
    ...
    await expect(
      page.getByText("A new version of the app is available."),
    ).toBeVisible();
    ...
    await page.waitForFunction(
      () =>
        (window as unknown as { __testMarker?: boolean }).__testMarker ===
        undefined,
    );
  ...
  test("Ignore dismisses the toast without reloading the page", async ({
    page,
  }) => {
    await page.goto("./");
    await page.evaluate(() => navigator.serviceWorker.ready);
